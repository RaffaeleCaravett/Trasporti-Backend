...
@PreAuthorize("hasAuthority('Azienda')")
   ...
   return annuncioService.deleteByAzienda(id,annuncioId);
    ...
    @GetMapping("/byRetribuzione/{retribuzione1}/{retribuzione2}/me")
    public Page<Annuncio> getByRetribuzione(@PathVariable int retribuzione1, @PathVariable int retribuzione2, @AuthenticationPrincipal Azienda azienda,@RequestParam(defaultValue = "0") int page, @RequestParam(defaultValue = "10") int size, @RequestParam(defaultValue = "id") String orderBy){
        return annuncioService.findByRetribuzione(retribuzione1,retribuzione2,azienda.getId(),page,size,orderBy);
    }
    @GetMapping("/byData/{anno1}/{mese1}/{giorno1}/{anno2}/{mese2}/{giorno2}/me")
    public Page<Annuncio> getByData(@PathVariable int anno1,@PathVariable int mese1,@PathVariable int giorno1,@PathVariable int anno2,@PathVariable int mese2,@PathVariable int giorno2,@AuthenticationPrincipal Azienda azienda,@RequestParam(defaultValue = "0") int page,@RequestParam(defaultValue = "10") int size,@RequestParam(defaultValue = "id") String orderBy){
        return annuncioService.findByData(anno1,mese1,giorno1,anno2,mese2,giorno2,azienda.getId(),page,size,orderBy);
    }
